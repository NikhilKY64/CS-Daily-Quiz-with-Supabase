...
export async function getQuizAttempts(userId) {
  if (typeof window === 'undefined') {
    ...
    // Client-side: prefer using the Supabase JS client directly (handles auth/RLS in-browser)
    try {
      const { data, error } = await supabase
        .from('quiz_attempts')
        .select('*')
      ...
      console.error('Error getting quiz attempts (client supabase):', error)
      // As a last-resort fallback try the REST endpoint with explicit logging
      try {
        const base = typeof process !== 'undefined' && process.env && process.env.NEXT_PUBLIC_SUPABASE_URL ? process.env.NEXT_PUBLIC_SUPABASE_URL : ''
        const url = `${base}/rest/v1/quiz_attempts?user_id=eq.${userId}&select=*&order=created_at.desc`
        if (!base) console.warn('NEXT_PUBLIC_SUPABASE_URL is not set; REST fetch may fail and create a relative /undefined URL')
        ...
        console.error('Fallback REST fetch for quiz_attempts failed:', fetchErr)
        throw fetchErr
      }
    }
  }
...
// Asked questions helpers
export async function getAskedQuestionIdsForUser(userId) {
  if (!userId) return []
  const { data, error } = await supabase.from('asked_questions').select('question_id').eq('user_id', userId)
  if (error) {
    console.error('Error fetching asked questions for user', userId, error)
    return []
  }
  return (data || []).map((r) => String(r.question_id))
}

export async function recordAskedQuestions(userId, questionIds, quizId = null) {
  if (!userId || !Array.isArray(questionIds) || questionIds.length === 0) return
  const rows = questionIds.map((qid) => ({ user_id: userId, question_id: String(qid), quiz_id: quizId }))
  const { error } = await supabase.from('asked_questions').insert(rows)
  if (error) {
    console.error('Failed to record asked questions for user', userId, error)
  }
}